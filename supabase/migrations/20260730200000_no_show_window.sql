-- Finestra No-show lato ristoratore: da inizio turno a inizio turno + 30 minuti.
-- Fonte di verità server-side (UTC via timestamptz, wall time interpretato in Europe/Rome).
...
BEGIN
  IF auth.uid() IS NULL OR auth.uid() IS DISTINCT FROM NEW.restaurant_id THEN
    RETURN NEW; -- lavoratore / admin / job automatici: non gestiti qui
  END IF;
  ...
  IF OLD.status <> 'scheduled' THEN
    RETURN NEW; -- turni conclusi/annullati/no_show già gestiti da altre regole
  END IF;
