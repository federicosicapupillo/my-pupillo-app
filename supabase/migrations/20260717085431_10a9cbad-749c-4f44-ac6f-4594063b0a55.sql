-- Seed feature flag `worker_tax_code_enabled` (global, ON by default)
INSERT INTO public.feature_flags (key, enabled, scope, description)
VALUES (
  'worker_tax_code_enabled',
  true,
  'global',
  'Codice fiscale lavoratori — mostra e rende obbligatorio il Codice Fiscale per gli utenti con ruolo lavoratore.'
)
ON CONFLICT (key) DO NOTHING;

-- Update enforce_worker_personal_data to gate tax_code on the feature flag.
-- When the flag is OFF:
--  * tax_code is NOT required for profile_completed
--  * no format validation is performed (NULL / empty accepted)
--  * existing values are preserved (no auto-nulling)
-- When ON: behavior is bit-identical to the previous trigger.
...
DECLARE
  ...
  has_residence_address boolean;
  doc_fields_changed boolean;
  require_doc boolean := public.is_feature_enabled('require_id_document');
  require_tax_code boolean := public.is_feature_enabled('worker_tax_code_enabled');
BEGIN
  ...
  has_residence_address :=
    (NEW.residence_address IS NOT NULL AND length(btrim(NEW.residence_address)) > 0)
    OR (
      NEW.residence_street IS NOT NULL AND length(btrim(NEW.residence_street)) > 0
      AND NEW.residence_number IS NOT NULL AND length(btrim(NEW.residence_number)) > 0
    );

  -- Completezza campi ANAGRAFICI non-documento: tax_code è condizionale al flag.
  IF NEW.first_name IS NULL OR length(btrim(NEW.first_name)) = 0
     ...
     OR (require_tax_code AND (NEW.tax_code IS NULL OR length(btrim(NEW.tax_code)) = 0))
     OR NEW.nationality IS NULL OR length(btrim(NEW.nationality)) = 0
     OR NOT has_residence_address
  ...
  THEN
    RAISE EXCEPTION 'Completa tutti i campi anagrafici obbligatori.';
  END IF;

  IF require_doc THEN
    IF NEW.id_document_type IS NULL OR NEW.id_document_type NOT IN ('carta_identita','passaporto','patente')
    ...
    doc_fields_changed := (TG_OP = 'INSERT') OR (
      TG_OP = 'UPDATE' AND (
        NEW.id_document_expires_at IS DISTINCT FROM OLD.id_document_expires_at
        OR NEW.id_document_issued_at IS DISTINCT FROM OLD.id_document_issued_at
        OR NEW.id_document_number    IS DISTINCT FROM OLD.id_document_number
        OR NEW.id_document_type      IS DISTINCT FROM OLD.id_document_type
        OR NEW.id_document_issuer    IS DISTINCT FROM OLD.id_document_issuer
      )
    );

    IF doc_fields_changed AND NEW.id_document_expires_at < today_it THEN
  ...
  -- Validazione formato CF: eseguita solo se il flag è ON *e* se un valore è
  -- presente. Quando OFF, il campo può essere NULL o vuoto senza errori;
  -- eventuali valori storici NON vengono cancellati automaticamente.
  IF require_tax_code AND NEW.tax_code IS NOT NULL AND length(btrim(NEW.tax_code)) > 0 THEN
