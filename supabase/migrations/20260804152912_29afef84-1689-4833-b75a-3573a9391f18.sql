...
BEGIN
  ...
  IF app.status NOT IN ('pending', 'interested', 'counter_offer') THEN
    -- Se la candidatura e' stata chiusa perche' il lavoratore risulta gia'
    -- confermato su un turno sovrapposto, restituiamo il codice specifico.
    IF app.announcement_id IS NOT NULL
       AND public.worker_conflicts_with_announcement(app.worker_id, app.announcement_id, NULL) THEN
