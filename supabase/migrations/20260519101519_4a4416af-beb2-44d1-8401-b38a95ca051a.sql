...
BEGIN
  ...
  -- Specializzazione: proposta di lavoro inviata dal ristoratore
  IF NEW.template_id = 'shift_proposal' AND NEW.sender_id = v_app.restaurant_id THEN
    v_title := 'Hai ricevuto una proposta di lavoro';
    v_body  := 'Un ristorante ti ha inviato una proposta per un turno.';
    v_type  := 'job_proposal_received';
  ELSE
    v_title := 'Nuovo messaggio';
    v_body  := 'Hai ricevuto un nuovo messaggio su Pupillo.';
    v_type  := 'new_message';
  END IF;
