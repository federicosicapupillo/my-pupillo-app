...
BEGIN
  ...
  -- Moderation gate: block if either party is hidden by moderation
  IF (SELECT bool_or(coalesce(moderation_hidden, false)) FROM public.profiles
      WHERE id = ANY(ARRAY[v_worker_id, v_restaurant_id])) THEN
