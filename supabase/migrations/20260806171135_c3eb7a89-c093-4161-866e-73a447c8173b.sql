...
BEGIN
  ...
  BEGIN
    ...
    IF stack ~ '(consume_credits|grant_credits|award_referral_credits|register_referral|delete_my_account|recompute_review_block|recompute_worker_penalty|recompute_worker_reputation|send_required_review_reminders|admin_set_moderation_hidden|handle_new_review|trg_clean_shift_after_penalty|claim_signup_role)\(' THEN
      RETURN NEW;
    END IF;
  END;
