...
export const Route = createFileRoute("/lavoratori")({
  head: () => ({
    meta: [
      { title: "Lavora nella ristorazione con Pupillo — Turni vicino a te" },
      {
        name: "description",
        content:
          "Candidati ai turni Horeca pubblicati da ristoranti e locali nella tua zona. Orari e compensi chiari, candidatura in pochi clic, recensioni che fanno crescere la tua reputazione.",
      },
    ],
  }),
  component: LavoratoriPage,
});

function LavoratoriPage() {
  return (
      ...
      <header className="sticky top-0 z-30 border-b bg-background/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <Link to="/auth"><Button variant="ghost" size="sm">Accedi</Button></Link>
            <Link to="/auth" search={{ role: "worker" } as never}>
              <Button size="sm" className="gap-1">Registrati <ArrowRight className="h-3.5 w-3.5" /></Button>
            </Link>
          </div>
        </div>
      </header>
